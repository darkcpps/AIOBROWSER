...
class MonochromeTab(QWidget):
    ...
    def on_search_result_selected(self, item: QListWidgetItem):
        ...
        # Run in background thread (pass prefill from search result if available)
        thread = threading.Thread(
            target=self._fetch_metadata_thread,
            args=(result_type, result_id, item_data.get("data"))
        )
        thread.daemon = True
        thread.start()

    def _fetch_metadata_thread(self, content_type: str, content_id: str, prefill: Optional[Dict] = None):
        ...
        try:
            ...
            if content_type == "track":
                ...
                # Merge prefill (search result) metadata when API returns sparse data
                if prefill:
                    try:
                        pre = self.monochrome_api._normalize_track(prefill) if hasattr(self.monochrome_api, "_normalize_track") else prefill
                        print(f"[MONOCHROME UI] Merging prefill metadata for track {content_id}")
                        # Title: prefer prefill if fetched title is placeholder or autogenerated
                        fetched_title = track.get("title") or ""
                        if pre.get("title") and (not fetched_title or fetched_title in (None, "Unknown") or fetched_title.startswith("Track ") or fetched_title.strip().endswith(str(content_id))):
                            print(f"[MONOCHROME UI] Replacing sparse title '{fetched_title}' with prefill '{pre.get('title')}'")
                            track["title"] = pre["title"]

                        # Artists: prefer prefill if fetched artists are missing or only 'Unknown Artist'
                        fetched_artists = track.get("artists") or []
                        def _is_unknown_artists(a):
                            if not a:
                                return True
                            if isinstance(a, list) and len(a) == 1 and isinstance(a[0], dict) and a[0].get("name", "").lower().startswith("unknown"):
                                return True
                            return False
                        if pre.get("artists") and _is_unknown_artists(fetched_artists):
                            print(f"[MONOCHROME UI] Replacing sparse artists {fetched_artists} with prefill {pre.get('artists')}")
                            track["artists"] = pre["artists"]

                        # Album: prefer prefill album if fetched album missing or unknown
                        fetched_album = track.get("album") or {}
                        fetched_album_title = fetched_album.get("title") if isinstance(fetched_album, dict) else None
                        if pre.get("album") and (not fetched_album_title or fetched_album_title in (None, "Unknown")):
                            track["album"] = pre["album"]

                        # Duration / ISRC
                        for simple_k in ("duration", "isrc"):
                            if (not track.get(simple_k)) and pre.get(simple_k):
                                track[simple_k] = pre[simple_k]
                    except Exception as e:
                        print(f"[MONOCHROME UI] Failed to merge prefill metadata: {e}")
                tracks = [track]
    ...
    def on_metadata_loaded(self, data: Dict):
        ...
        # Clear and populate track list (skip any tracks without an ID)
        self.track_list.clear()
        valid_tracks = []
        skipped = 0
        for track in tracks:
            # Resolve track id from common fields
            track_id = (
                track.get("id") or track.get("uuid") or track.get("trackId") or
                (track.get("item") or {}).get("id") or (track.get("track") or {}).get("id")
            )
            if not track_id:
                print(f"[MONOCHROME UI] Skipping track with missing id: {track.get('title', 'Unknown')}")
                skipped += 1
                continue

            # Ensure track dict has an 'id' for downstream code
            track["id"] = track_id
        ...
        # Replace current tracks with validated list
        self.current_tracks = valid_tracks

        status_text = f"✅ Loaded {len(valid_tracks)} track(s)"
        if skipped:
            status_text += f" • Skipped {skipped} track(s) with missing id"
    ...
    def on_download_error(self, track_id: str, error_msg: str):
        ...
        if track_id:
            for i in range(self.track_list.count()):
                ...
                if str(item.data(Qt.ItemDataRole.UserRole)) == track_id:
                    ...
                    # attach full error in tooltip for debugging
        ...
        if "Not Found" in error_msg or "404" in error_msg:
            user_msg = "Track not available via Monochrome API (Not found)."
        elif "Upstream API error" in error_msg or "403" in error_msg:
            user_msg = "Track unavailable due to upstream API error (restricted)."

        self.status_label.setText(f"❌ Error: {user_msg}")
        self.status_label.setStyleSheet(f"color: {COLORS.get('accent_red', '#ff0000')}; font-size: 12px;")
        self.download_btn.setEnabled(True)
