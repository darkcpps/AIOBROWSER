"""
Tidal Downloader Tab
UI for downloading high-quality music from Tidal (FLAC up to 24-bit/192kHz)
"""
import threading
...
from core.tidal_downloader import TidalAPI, TidalDownloader, MetadataHelper, AudioQuality
...
class TidalTab(QWidget):
    ...
    def init_ui(self):
        ...
        # Title
        title = QLabel("🎵 Tidal Downloader")
        title.setStyleSheet(f"font-size: 24px; font-weight: bold; color: {COLORS['text_primary']};")
        layout.addWidget(title)

        # Info banner
        info_banner = QLabel("✨ Download lossless music from Tidal • FLAC up to 24-bit/192kHz • No account required\n⚠️ Note: Some tracks may be unavailable due to region restrictions or limited API coverage")
        ...
        # Status Label
        self.status_label = QLabel("Search for music or enter a Tidal URL to begin")
    ...
    def create_url_section(self) -> QGroupBox:
        """Create Tidal URL/search input section"""
        group = QGroupBox("Search or URL")
        ...
        self.url_input.setPlaceholderText("Search by title or paste Tidal URL...")
        self.url_input.returnPressed.connect(self.search_or_fetch)
        ...
        """)
        search_btn.clicked.connect(self.search_or_fetch)
    ...
    def search_or_fetch(self):
        ...
        if not input_text:
            self.status_label.setText("❌ Please enter a search term or Tidal URL")
            self.status_label.setStyleSheet(f"color: {COLORS.get('accent_red', '#ff0000')}; font-size: 12px;")
            return

        # Check if it's a URL
        if "tidal.com" in input_text or "listen.tidal.com" in input_text:
            self.fetch_metadata(input_text)
        else:
            self.search_tidal(input_text)

    def search_tidal(self, query: str):
        """Search Tidal catalog"""
        self.status_label.setText("🔍 Searching Tidal...")
        self.status_label.setStyleSheet(f"color: {COLORS['text_secondary']}; font-size: 12px;")
    ...
    def _search_thread(self, query: str, search_type: str):
        ...
        try:
            print(f"[TIDAL UI] Searching for '{query}' (type: {search_type})")
            results = self.tidal_api.search(query, search_type, limit=50)
            print(f"[TIDAL UI] Search complete: {results}")

            self.search_results_loaded.emit({"query": query, "type": search_type, "results": results})

        except Exception as e:
            print(f"[TIDAL UI] Search ERROR: {str(e)}")
            import traceback
            traceback.print_exc()
    ...
    def on_search_results_loaded(self, data: Dict):
        ...
        if search_type == "tr":  # Tracks
            ...
            for item in items:
                ...
                list_item = QListWidgetItem(text)
                list_item.setData(Qt.ItemDataRole.UserRole, {"type": "playlist", "id": item["id"], "data": item})
                self.search_results_list.addItem(list_item)
                self.search_results.append(item)

        elif search_type == "ar":  # Artists
    ...
    def on_search_result_selected(self, item: QListWidgetItem):
        ...
        print(f"[TIDAL UI] Selected {result_type}: {result_id}")

        # Build URL for the selected item
        url = f"https://tidal.com/browse/{result_type}/{result_id}"
        self.fetch_metadata(url)

    def fetch_metadata(self, url: str):
        """Fetch metadata from Tidal URL"""
        if not url:
            url = self.url_input.text().strip()

        if not url:
            self.status_label.setText("❌ Please enter a Tidal URL")
            self.status_label.setStyleSheet(f"color: {COLORS.get('accent_red', '#ff0000')}; font-size: 12px;")
            return
    ...
    def _fetch_metadata_thread(self, url: str):
        ...
        try:
            print(f"[TIDAL UI] Starting metadata fetch for URL: {url}")
            tidal_type, tidal_id = self.tidal_api.parse_tidal_url(url)
            print(f"[TIDAL UI] Parsed type: {tidal_type}, ID: {tidal_id}")

            if tidal_type == "track":
                print(f"[TIDAL UI] Fetching single track...")
                track = self.tidal_api.get_track(tidal_id)
                tracks = [track]
            elif tidal_type == "album":
                print(f"[TIDAL UI] Fetching album...")
                album = self.tidal_api.get_album(tidal_id)
                tracks = album.get("tracks", [])
                print(f"[TIDAL UI] Album has {len(tracks)} tracks")
            ...
            elif tidal_type == "playlist":
                print(f"[TIDAL UI] Fetching playlist...")
                playlist = self.tidal_api.get_playlist(tidal_id)
                tracks = playlist.get("tracks", [])
                print(f"[TIDAL UI] Playlist has {len(tracks)} tracks")
            elif tidal_type == "artist":
                print(f"[TIDAL UI] Fetching artist top tracks...")
                # Get artist's top tracks
                top_tracks = self.tidal_api.get_artist_top_tracks(tidal_id, limit=20)
                tracks = top_tracks
                print(f"[TIDAL UI] Artist has {len(tracks)} top tracks")
            else:
                raise ValueError(f"Unsupported Tidal type: {tidal_type}")

            print(f"[TIDAL UI] Emitting metadata for {len(tracks)} tracks")
            self.metadata_loaded.emit({"type": tidal_type, "tracks": tracks})

        except Exception as e:
            print(f"[TIDAL UI] ERROR: {str(e)}")
            import traceback
            traceback.print_exc()
    ...
    def start_downloads(self):
        ...
        if not self.downloader:
            self.downloader = TidalDownloader(self.tidal_api)

        # Get download path from settings
        download_path = self.parent.settings_manager.get("download_path", str(Path.home() / "Downloads"))
        tidal_path = Path(download_path) / "Tidal"
        tidal_path.mkdir(parents=True, exist_ok=True)
    ...
    def _download_track_thread(self, track_data: Dict, output_dir: Path):
        ...
        try:
            ...
            print(f"[TIDAL UI] Download thread started for: {track_title} (ID: {track_id})")

            def progress_callback(progress, downloaded, total):
                self.download_progress.emit(track_id, progress)

            print(f"[TIDAL UI] Calling downloader.download_track...")
            ...
            print(f"[TIDAL UI] Download complete, result: {result}")
            self.download_complete.emit(track_id, result)

        except Exception as e:
            print(f"[TIDAL UI] Download error: {str(e)}")
            import traceback
            traceback.print_exc()
