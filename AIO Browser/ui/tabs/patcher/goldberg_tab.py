...
class GoldbergTab(QWidget):
    ...
    def initUI(self):
        ...
        if_title.setStyleSheet(f"font-size: 18px; font-weight: bold; color: {COLORS['text_primary']};")
        ...
        if_content.setStyleSheet(f"color: {COLORS['text_secondary']}; font-size: 14px; line-height: 1.6;")
        ...
        def toggle_info():
            ...
            self.toggle_info_btn.setText("❌ Close Info" if self.info_visible else "❔ What is Goldberg?")
    ...
    def manual_folder_patch(self):
        folder = QFileDialog.getExistingDirectory(self, "Select Game Folder (must contain steam_api.dll or steam_api64.dll)")
