...
import requests
from core import downloader, patcher, scraper, steam_utils
...
class GameSearchApp(QMainWindow):
    ...
    def show_main_interface(self):
        ...
        self.sidebar.setStyleSheet(
            f"QFrame#Sidebar {{ background-color: {COLORS['bg_secondary']}; border-right: 1px solid {COLORS['border']}; }}"
        )
    ...
    def setup_search_tab(self):
        ...
        self.direct_search_bar = QFrame()
        self.direct_search_bar.setFixedHeight(60)
        self.direct_search_bar.setStyleSheet(f"""
            background-color: {COLORS["bg_secondary"]};
            border: 1px solid {COLORS["border"]};
            border-radius: 12px;
        """)
        direct_sb_layout = QHBoxLayout(self.direct_search_bar)
        direct_sb_layout.setContentsMargins(10, 5, 10, 5)
        direct_sb_layout.setSpacing(10)

        self.direct_search_input = QLineEdit()
        self.direct_search_input.setPlaceholderText(
            "Search for direct download games..."
        )
        self.direct_search_input.setFixedHeight(45)
        self.direct_search_input.setStyleSheet(
        ...
        direct_layout.addWidget(self.direct_search_bar)
        ...
        self.torrent_search_bar = QFrame()
        self.torrent_search_bar.setFixedHeight(60)
        self.torrent_search_bar.setStyleSheet(f"""
            background-color: {COLORS["bg_secondary"]};
            border: 1px solid {COLORS["border"]};
            border-radius: 12px;
        """)
        torrent_sb_layout = QHBoxLayout(self.torrent_search_bar)
        torrent_sb_layout.setContentsMargins(10, 5, 10, 5)
        torrent_sb_layout.setSpacing(10)

        self.torrent_search_input = QLineEdit()
        self.torrent_search_input.setPlaceholderText(
            "Search for torrent/fitgirl repacks..."
        )
        self.torrent_search_input.setFixedHeight(45)
        self.torrent_search_input.setStyleSheet(
            "border: none; background: transparent; padding: 0 15px; font-size: 16px;"
        )
        ...
        torrent_layout.addWidget(self.torrent_search_bar)

        self.torrent_scroll = QScrollArea()
        self.torrent_scroll.setWidgetResizable(True)
        # Create a torrent results widget
        ...
        # Setup glow animations for search bars
    ...
    def setup_search_bar_animations(self):
        """Setup glow animation timers for search bars"""
        self.direct_glow_timer = QTimer()
        self.direct_glow_timer.setInterval(50)
        self.direct_glow_value = 0
        self.direct_glow_direction = 1
        self.direct_glow_timer.timeout.connect(self.animate_direct_glow)

        self.torrent_glow_timer = QTimer()
        self.torrent_glow_timer.setInterval(50)
        self.torrent_glow_value = 0
        self.torrent_glow_direction = 1
        self.torrent_glow_timer.timeout.connect(self.animate_torrent_glow)

        # Create glow effects once
    ...
    def animate_direct_glow(self):
        """Animate the glow effect for direct search bar"""
        try:
            self.direct_glow_value += self.direct_glow_direction * 5
            if self.direct_glow_value >= 100:
                self.direct_glow_value = 100
                self.direct_glow_direction = -1
            elif self.direct_glow_value <= 0:
                self.direct_glow_value = 0
                self.direct_glow_direction = 1
            ...
            # Apply glow effect safely
            if (
                hasattr(self, "direct_glow_effect")
                and self.direct_glow_effect is not None
            ):
                try:
                    self.direct_glow_effect.setBlurRadius(shadow_blur)
                    self.direct_glow_effect.setColor(
                        QColor(124, 58, 237, int(100 + glow_intensity * 155))
                    )
            ...
            print(f"[DEBUG] Direct glow animation error: {e}")

    def animate_torrent_glow(self):
        """Animate the glow effect for torrent search bar"""
        try:
            self.torrent_glow_value += self.torrent_glow_direction * 5
            if self.torrent_glow_value >= 100:
                self.torrent_glow_value = 100
                self.torrent_glow_direction = -1
            elif self.torrent_glow_value <= 0:
                self.torrent_glow_value = 0
                self.torrent_glow_direction = 1
            ...
            # Apply glow effect safely
            if (
                hasattr(self, "torrent_glow_effect")
                and self.torrent_glow_effect is not None
            ):
                try:
                    self.torrent_glow_effect.setBlurRadius(shadow_blur)
                    self.torrent_glow_effect.setColor(
                        QColor(124, 58, 237, int(100 + glow_intensity * 155))
                    )
            ...
            print(f"[DEBUG] Torrent glow animation error: {e}")

    def start_direct_glow(self):
        """Start glowing animation for direct search bar"""
        self.direct_glow_value = 0
        self.direct_glow_direction = 1
        # Recreate effect to ensure it's fresh
        self.direct_glow_effect = QGraphicsDropShadowEffect()
        self.direct_search_bar.setGraphicsEffect(self.direct_glow_effect)
        self.direct_glow_timer.start()

    def stop_direct_glow(self):
        """Stop glowing animation for direct search bar"""
        ...
        # Remove glow effect
        try:
            self.direct_search_bar.setGraphicsEffect(None)
            self.direct_glow_effect = None
        except:
            pass

    def start_torrent_glow(self):
        """Start glowing animation for torrent search bar"""
        self.torrent_glow_value = 0
        self.torrent_glow_direction = 1
        # Recreate effect to ensure it's fresh
        self.torrent_glow_effect = QGraphicsDropShadowEffect()
    ...
    def stop_torrent_glow(self):
        """Stop glowing animation for torrent search bar"""
        ...
        # Remove glow effect
        try:
            self.torrent_search_bar.setGraphicsEffect(None)
            self.torrent_glow_effect = None
        except:
            pass
    ...
    def setup_creamapi_subtab(self):
        ...
        self.steam_search_input_creamapi.setPlaceholderText(
            "Filter your Steam library..."
        )
        self.steam_search_input_creamapi.setFixedHeight(45)
        self.steam_search_input_creamapi.setStyleSheet(
            f"background-color: {COLORS['bg_secondary']}; border-radius: 12px; padding: 0 15px;"
        )
        self.steam_search_input_creamapi.textChanged.connect(
            self.filter_steam_games_creamapi
        )
        ...
        self.steam_scroll_creamapi.setStyleSheet(
            "background: transparent; border: none;"
        )

        self.steam_container_creamapi = QWidget()
        self.steam_container_layout_creamapi = QVBoxLayout(
            self.steam_container_creamapi
        )
    ...
    def load_steam_games(self):
        ...
        def run_scan():
            try:
                ...
                QMetaObject.invokeMethod(
                    self, "display_steam_games", Qt.ConnectionType.QueuedConnection
                )
            except Exception as e:
    ...
    def filter_steam_games(self, text):
        filtered = [
            g for g in self.installed_steam_games if text.lower() in g["name"].lower()
        ]
    ...
    def render_steam_games(self, games):
        ...
        for game in games:
            ...
            name.setStyleSheet(
                f"font-size: 16px; font-weight: 800; color: {COLORS['text_primary']}; background: transparent;"
            )
            info.addWidget(name)
            ...
            appid.setStyleSheet(
                f"font-size: 11px; color: {COLORS['accent_secondary']}; background: transparent; font-weight: 600;"
            )
        ...
        if msg_box.exec() == QMessageBox.StandardButton.Yes:
            ...
            def run():
                ...
                success, log_msg = patcher.patch_game(
                    game["full_path"], game["id"], "tools", nickname=nick, language=lang
                )
                QMetaObject.invokeMethod(
                    self,
                    "on_patch_finished",
    ...
    def filter_steam_games_creamapi(self, text):
        filtered = [
            g
            for g in self.installed_steam_games_creamapi
            if text.lower() in g["name"].lower()
        ]
    ...
    def render_steam_games_creamapi(self, games):
        ...
        for game in games:
            ...
            revert_btn.clicked.connect(
                lambda checked, g=game: self.trigger_revert_creamapi(g)
            )
            btn_layout.addWidget(revert_btn)
            ...
            apply_btn.clicked.connect(
                lambda checked, g=game: self.trigger_patch_creamapi(g)
            )
            btn_layout.addWidget(apply_btn)
    ...
    def trigger_patch_creamapi(self, game):
        ...
        if dialog.exec() == QDialog.DialogCode.Accepted:
            ...
            dlc_ids = (
                [line.strip() for line in dlc_text.split("\n") if line.strip()]
                if dlc_text
                else None
            )
            ...
            msg_box.setInformativeText(
                "This will replace original Steam DLLs with CreamAPI versions (backups will be created)."
            )
            msg_box.setStandardButtons(
                QMessageBox.StandardButton.Yes | QMessageBox.StandardButton.No
            )
            ...
            if msg_box.exec() == QMessageBox.StandardButton.Yes:
                ...
                self.statusBar().showMessage(
                    f"Patching {game['name']} with CreamAPI..."
                )

                def run():
                    success, log_msg = patcher.patch_game_creamapi(
                        game["full_path"], game["id"], "tools", dlc_ids=dlc_ids
                    )
                    QMetaObject.invokeMethod(
                        self,
                        "on_patch_finished_creamapi",
                        Qt.ConnectionType.QueuedConnection,
                        Q_ARG(bool, success),
                        Q_ARG(str, log_msg),
                    )
    ...
    def trigger_revert_creamapi(self, game):
        ...
        msg_box.setText(f"Restore original files for {game['name']}?")
        msg_box.setInformativeText(
            "This will restore the .bak files and remove CreamAPI configuration. It only works if a backup exists."
        )
        msg_box.setStandardButtons(
            QMessageBox.StandardButton.Yes | QMessageBox.StandardButton.No
        )
    ...
    def start_direct_search(self):
        ...
        # Show loading widget at top
        self.direct_loading_widget = LoadingWidget("Searching AnkerGames")
        self.direct_results_layout.addWidget(
            self.direct_loading_widget, alignment=Qt.AlignmentFlag.AlignHCenter
        )
        ...
        threading.Thread(
            target=self.perform_direct_search, args=(query,), daemon=True
        ).start()
    ...
    def render_direct_page(self):
        ...
        self.create_pagination_controls(
            self.direct_results_layout,
            len(self.direct_results),
            self.direct_current_page,
    ...
    def start_torrent_search(self):
        ...
        self.torrent_results_layout.addWidget(
            self.torrent_loading_widget, alignment=Qt.AlignmentFlag.AlignHCenter
        )
        ...
        threading.Thread(
            target=self.perform_torrent_search, args=(query,), daemon=True
        ).start()
    ...
    def render_torrent_page(self):
        ...
        # Pagination controls
        self.create_pagination_controls(
            self.torrent_results_layout,
            len(self.torrent_results),
            self.torrent_current_page,
            self.torrent_page_size,
